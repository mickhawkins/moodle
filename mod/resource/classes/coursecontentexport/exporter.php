...
/**
 * Course content exporter implementation for mod_resource.
 *
 * @package mod_resource\coursecontentexport
 * @copyright 2020 Simey Lameze <simey@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
namespace mod_resource\coursecontentexport;
...
class exporter extends basecontentexporter {
    ...
    public static function export_contentitems_for_user(array $contentitems, stdClass $user, zipwriter $archive): array {
        ...
        foreach ($contentitems as $item) {
            ...
            $filepathnames = self::export_resource_content($item->get_context(), $file, $archive);
     ...
     * @return array
     */
    protected static function export_resource_content(context $context, stored_file $file, zipwriter $archive): array {
        $fs = get_file_storage();
        ...
        if ($file->is_directory()) {
            ...
            foreach ($files as $dirfile) {
                $pathnames[] = self::export_resource_content($file, $archive);
            }
        } else {
