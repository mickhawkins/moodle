...
class core_calendar_renderer extends plugin_renderer_base {
    ...
    public function fake_block_threemonths(calendar_information $calendar) {
        ...
        list($previousmonth, ) = calendar_get_view($calendar, 'minithree', false, true, null, CALENDAR_LIMIT_DAY_EVENTS);

        // Current month.
        $calendar->set_time($current);
        list($currentmonth, ) = calendar_get_view($calendar, 'minithree', false, true, null, CALENDAR_LIMIT_DAY_EVENTS);

        // Next month.
        $calendar->set_time($next);
        list($nextmonth, ) = calendar_get_view($calendar, 'minithree', false, true, null, CALENDAR_LIMIT_DAY_EVENTS);
    ...
    public function subscription_details($unused, $subscriptions, $unused2 = '') {
        ...
        foreach ($subscriptions as $sub) {
            ...
            $tablerow = new html_table_row(array(
                ...
                new html_table_cell($this->subscription_action_links($sub))
            ));
            $tablerow->attributes += [
                'data-subid' => $sub->id,
    ...
    protected function render_subscription_update_interval(stdClass $subscription): string {
        ...
        if (!empty($subscription->url)) {
            ...
            $output .= html_writer::start_tag('select', [
                'id' => 'sub-interval-for-' . $subscription->id,
                'name' => 'pollinterval',
                'class' => 'custom-select',
                'aria-label' => get_string('pollinterval', 'calendar')
            ]);
     ...
     * @return string
     */
    protected function subscription_action_links($subscription): string {
        $html = html_writer::start_tag('div', array('class' => 'btn-group float-left'));
        if (!empty($subscription->url)) {
            $html .= html_writer::span(html_writer::link('#', get_string('update'),
                ['data-action' => 'update-subscription']), 'pr-2');
        }
