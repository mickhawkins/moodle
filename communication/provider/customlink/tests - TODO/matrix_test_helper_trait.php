...
trait matrix_test_helper_trait {
    ...
    protected function initialise_mock_configs(): void {
        $this->matrixhomeserverurl = TEST_communication_customlink_MOCK_SERVER;
        set_config('matrixhomeserverurl', $this->matrixhomeserverurl, 'communication_customlink');
        ...
        set_config('matrixaccesstoken', $this->accesstoken, 'communication_customlink');
    }
    ...
    protected function initialise_mock_server(): void {
        if (!defined('TEST_communication_customlink_MOCK_SERVER')) {
            $this->markTestSkipped(
                'The TEST_communication_customlink_MOCK_SERVER constant must be defined to run communication_customlink tests'
            );
        }
        $this->reset_mock();
    ...
    public function reset_mock(): void {
        if (defined('TEST_communication_customlink_MOCK_SERVER')) {
            $request = $this->request();
            $response = $request->post(TEST_communication_customlink_MOCK_SERVER. '/backoffice/reset');
            $response = json_decode($response->getBody());
