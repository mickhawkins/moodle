...
const handleAuthorization = (issuerId) => {
    ...
    return MoodleNetRepository.authorizationCheck(issuerId, courseId).then(async(data) => {
        if (!data.status) {
            // Not yet authorized.
            ...
            return window.open(data.loginurl, 'moodlenet_auth',
                `location=0,status=0,width=${data.windowsizewidth},height=${data.windowsizeheight},scrollbars=yes`);
        } else {
            // Already authorized.
                    ...
                    if (data.status) {
                        ...
                        const modalPromise = ModalFactory.create({
                            ...
                            templateContext: {
                                ...
                                'sharetype': await getString('moodlenet:share_type_' + shareType, 'moodle'),
                                'server': data.server,
                            }
                        });
