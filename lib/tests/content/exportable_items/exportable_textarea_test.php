...
class exportable_textarea_test extends advanced_testcase {
    ...
    public function test_valid_table_with_content_no_filearea_specified(): void {
        ...
        $this->assert_exportable_matches_file($component, $user, $context, null, $filepath, $content, [], $exportable);
    }
...
EOF;
        ...
        $this->assert_exportable_matches_file(
            $component, $user, $context, $filearea, $filepath, $expectedcontent, $expectedfiles, $exportable
        );
    }

    /**
     * Ensure that serialisation of all properties returns an object which exactly matches and can be used.
     */
...
EOF;
        ...
        // Serialize and unserialize the item
        $serialized = serialize($exportable);
        $unserialized = unserialize($serialized);
        $this->assertEquals($exportable, $unserialized);
    ...
    /**
     * Get a mocked export_controller.
     *
     * @param   string $component
     * @param   stdClass $user
     ...
     */
    protected function get_mocked_export_controller(string $component, stdClass $user, context $rootcontext, zipwriter $archive): export_controller {
        return $this->getMockForAbstractClass(export_controller::class, [
                $component,
                $user,
                $rootcontext,
                $archive
    ...
    protected function assert_exportable_matches_file(
        ...
        $archive = $this->getMockBuilder(zipwriter::class)
            ...
            ->setMethods([
                'is_file_in_archive',
                'add_file_from_html_string',
                'add_file_from_stored_file',
            ])
            ->getMock();
        $controller = $this->get_mocked_export_controller($component, $user, $context, $archive);

        $filepath = preg_replace('#/+#', '/', $filename);
        ...
        foreach ($expectedfiles as $file) {
            $filepathinzip = dirname($filepath) . $file->get_filearea() . '/' . $file->get_filepath() . $file->get_filename();
        ...
        $archive->expects($this->once())
            ->method('add_file_from_html_string')
            ->with(
                $this->equalTo($context),
        ...
        $exportable->add_to_archive($controller);
    }
